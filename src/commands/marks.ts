...
import { bullet, percent, points } from "../lib/format.js";

export default class Marks extends BaseCommand {
  static override aliases = ["grades", "grade"];
  static override summary = "Show submitted marks and bounded grade estimates";
  static override description = "Show marked assessments, Canvas current scores when available, and explicitly limited local estimates.";
  static override examples = ["<%= config.bin %> marks", "<%= config.bin %> marks --course CHEM101", "<%= config.bin %> grades --json"];
  static override flags = { course: Flags.string({ char: "c", description: "limit to one course code or id" }) };
  ...
  async run(): Promise<unknown> {
    ...
    const dto = {
      ...
      marks: result.items.map(({ course, assignment, submission, proctored }) => ({
        ...
        percent: submission.score != null && assignment?.points_possible
          ? Number(((submission.score / assignment.points_possible) * 100).toFixed(2))
          : null,
    ...
    if (!this.jsonEnabled()) {
      for (const projection of result.projections) {
        const current = projection.currentGrade == null
          ? "no reliable current score"
          : `${projection.currentGrade.toFixed(1)}% ${projection.basis === "canvas" ? "Canvas current score" : "simple current estimate"}`;
        const contribution = projection.finalLockedIn == null
          ? "recorded weighted contribution unavailable"
          : `${projection.finalLockedIn.toFixed(1)} percentage points recorded by the simple weighting model`;
        this.log(`${projection.course.code} · ${current} · ${contribution}`);
        for (const limitation of projection.limitations) this.log(`  limitation: ${limitation}`);
      }
