...
export default class Library extends BaseCommand {
  ...
  static override description = "Use `library search` or `library fetch`. Library requests never receive the Canvas token.";
  static override examples = ["<%= config.bin %> library search referencing", "<%= config.bin %> library fetch https://library.example.edu"];
