...
export default class Page extends BaseCommand {
  ...
  static override description = "Fetch an individual Canvas page. The pages index is intentionally not used; get slugs from `easel modules`.";
  ...
  async run(): Promise<unknown> {
    ...
    const dto = {
      ...
      slug: page.url,
      title: page.title,
      text: flags.full || flags.output || flags.objective ? renderContent({ title: page.title, text }, flags) : renderContent({ title: page.title, text }, flags),
      files: links.map((link) => ({ courseId: link.courseId, fileId: link.fileId, verifier: link.verifier, label: link.label, url: link.url })),
    };
